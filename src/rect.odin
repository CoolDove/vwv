...
rect_position :: #force_inline proc(r: R) -> dd.Vec2 {
    return {r.x, r.y}
}
rect_size :: #force_inline proc(r: R) -> dd.Vec2 {
    return {r.w, r.h}
}

rect_require :: proc(r: R, width:f32=0, height:f32=0, anchor:dd.Vec2={0,0}) -> R {
    if width <= r.w && height <= r.h do return r
    width := max(width, r.w)
    height := max(height, r.h)
    c := rect_position(r) + anchor * rect_size(r)
    scale :dd.Vec2= {width/r.w, height/r.h}
