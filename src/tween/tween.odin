...
TweenHandle :: #type hla.HollowArrayHandle(Tween)
...
tween_handle :: proc(tweener: ^Tweener, value: ^$T, target : T, duration : f32, easing_proc := ease_linear) -> TweenHandle {
	...
	using hla
	handle := hla_append(&tweener.tweens, Tween{})
	tween :^Tween= hla_get_pointer(handle)
...
tween :: proc(tweener: ^Tweener, value: ^$T, target : T, duration : f32, easing_proc := ease_linear) -> ^Tween {
	handle := tween_handle(tweener, value, target, duration, easing_proc)
